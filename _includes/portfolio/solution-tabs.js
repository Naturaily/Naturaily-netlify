  ...
  $tabsNavItems.click((event) => {
    ...
    if (!$newNav.data('active-nav') && !isAnimationGoing) {
      ...
      if (animationType === 'slide') {
        ...
        $activeNav.removeClass('portfolio-issues__nav-list__item--active');
        $newNav.addClass('portfolio-issues__nav-list__item--active');
        $activeTab.animate({ opacity: 0 }, animationTime/2, () => {
          $activeTab
            .css('display', 'none')
            .removeClass('portfolio-issues__tabs-tab--active')
            .attr('data-active-tab', 'false');
          $newTab
            .css('display', 'block')
            .animate({ opacity: 1 }, animationTime/2, () => {
              $newTab.addClass('portfolio-issues__tabs-tab--active');
            });
          updateActiveItems();
        });
      }
