const goalConfettiTimeline = gsap
  .timeline({ paused: true, repeat: -1, repeatDelay: 4 })
...
const solutionConfettiTimeline = gsap
  .timeline({ paused: true, repeat: -1, repeatDelay: 4 })
...
ScrollTrigger.matchMedia({
  ...
  "(min-width: 992px)": () => {
    ...
    gsap
      ...
      .from('[data-gsap="results-man"]', { duration: .6, opacity: 0, x: -150, y: -200, ease: "power3.in" })
      .from('[data-gsap="results-woman"]', { duration: .6, opacity: 0, rotation: -45, x: 50, ease: "power3.in" }, "-=.6")
    ;

    const goalPictureTl = gsap
      .timeline({ scrollTrigger: { trigger: '[data-gsap="picture"]' } })
    ...
    const solutionPeopleTl =  gsap
      .timeline({ scrollTrigger: { trigger: '[data-gsap="solution-people"]', start: "-300px center" } })
